...
#### Rectangle class
In the Rectangle object is created, it was initialized with `width` and `height` attributes. The class also contain the following methods:
* `set_width`
* `set_height`
...
Additionally, if an instance of a Rectangle is represented as a string, it is like: `Rectangle(width=5, height=10)
...
The Square class is able to access the Rectangle class methods also contains a `set_side` method. If an instance of a Square is represented as a string, it should look like: `Square(side=9)`
